...
export class NonefinityClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (this.debug) {
        console.log(`[NonefinityClient] Request: ${options.method || "GET"} ${url}`);
      }
  ...
  async listConfigs(
    ...
    return this.request<ChatConfigListResponse>(
      `/api/chats/configs?skip=${skip}&limit=${limit}`
    );
  }
  ...
  async getConfig(id: string): Promise<ApiResponse<ChatConfig>> {
    return this.request<ChatConfig>(`/api/chats/configs/${id}`);
  }
  ...
  async createConfig(
    ...
    return this.request<ChatConfig>("/api/chats/configs", {
      method: "POST",
  ...
  async deleteConfig(id: string): Promise<ApiResponse<void>> {
    return this.request<void>(`/api/chats/configs/${id}`, {
      method: "DELETE",
    });
  }
  ...
  async listSessions(
    ...
    return this.request<ChatSessionListResponse>(
      `/api/chats/sessions?skip=${skip}&limit=${limit}`
    );
  }
  ...
  async getSession(
    ...
    return this.request<ChatSession>(
      `/api/chats/sessions/${id}?skip=${skip}&limit=${limit}`
    );
  }
  ...
  async createSession(
    ...
    return this.request<ChatSession>("/api/chats/sessions", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }
  ...
  async deleteSession(id: string): Promise<ApiResponse<void>> {
    return this.request<void>(`/api/chats/sessions/${id}`, {
      method: "DELETE",
    });
  }
  ...
  async clearSessionMessages(id: string): Promise<ApiResponse<void>> {
    return this.request<void>(`/api/chats/sessions/${id}/messages`, {
      method: "DELETE",
    });
  }
  ...
  async streamMessage(
    ...
    try {
      ...
      const url = `${this.apiUrl}/api/chats/sessions/${sessionId}/stream`;
      if (this.debug) {
   ...
   */
  private processSSELine(line: string, onEvent: (event: StreamEvent) => void): void {
    ...
    if (eventMatch || dataMatch) {
      ...
      try {
        ...
        if (this.debug) {
          console.error("[NonefinityClient] Failed to parse SSE data:", e);
        }
        onEvent({
          event: eventType,
   ...
   */
  private processSSEBuffer(buffer: string, onEvent: (event: StreamEvent) => void): void {
  ...
  async saveConversation(
    ...
    return this.request<{ saved: number }>(
      `/api/chats/sessions/${sessionId}/save-conversation`,
      {
        method: "POST",
  ...
  async createAPIKey(
    ...
    return this.request<APIKeyCreateResponse>("/api/api-keys", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }
  ...
  async listAPIKeys(
    ...
    return this.request<APIKeyListResponse>(
      `/api/api-keys?skip=${skip}&limit=${limit}&include_inactive=${includeInactive}`
    );
  }
  ...
  async getAPIKey(keyId: string): Promise<ApiResponse<APIKeyResponse>> {
    return this.request<APIKeyResponse>(`/api/api-keys/${keyId}`);
  }
  ...
  async revokeAPIKey(keyId: string): Promise<ApiResponse<APIKeyResponse>> {
    return this.request<APIKeyResponse>(`/api/api-keys/${keyId}/revoke`, {
      method: "POST",
    });
  }
